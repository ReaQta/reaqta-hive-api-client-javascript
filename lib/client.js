...
const { PUT, POST, DELETE } = require('./methods')
...
class ReaQtaClient {
  ...
  constructor(config) {
    ...
    // Bind response and error wrappers to this instance
    this._wrapResponse = this._wrapResponse.bind(this)
    this._wrapError = this._wrapError.bind(this)
  ...
  _signAndExecuteRequest(url, requestOptions = {}) {
    ...
    const handleResult = ([res, tokenOptions]) => {
      ...
      this._jwtExpiresAtMS = tokenOptions.jwtExpiresAtMS
      return res
    }
  ...
  getTokenOptions() {
    const tokenOptions = {
      ...
      jwt: this._jwt,
      jwtExpiresAtMS: this._jwtExpiresAtMS
    }
    return tokenOptions
  }
  ...
  /**
   * Isolate an endpoint. (As of v3.9.0, this is only supported on Windows machines.)
   * @param {string} id - The id of the endpoint
   * @return {Promise<ReaQtaResponse>}
   * @rqtResource Endpoint
   */
  ...
  /**
   * De-isolate an endpoint. (As of v3.9.0, this is only supported on Windows machines.)
   *
   * NOTE - Added in v3.9.0. Previously, only isolation was supported with external APIs.
   *
   * @param {string} id - The id of the endpoint
   * @return {Promise<ReaQtaResponse>}
   * @rqtResource Endpoint
   */
  deisolateEndpoint(id) {
    return this._signAndExecuteRequest(
      `/1/endpoint/${id}/deisolate`,
      { method: POST }
    )
  }
   ...
   *
   * @typedef {Object} AlertOnProcessHashOptions
   ...
   *
   * @param {AlertOnProcessHashOptions} blacklistOptions - The options for the blacklist policy you wish to create
   * @return {Promise<ReaQtaResponse>}
   * @rqtResource Policy
   */
  triggerAlertOnProcessHash(blacklistOptions) {
    return this._signAndExecuteRequest(
      '/1/policy/trigger-alert-on-process-hash',
      {
        method: POST,
  ...
  /**
   * Details for the whitelist hash trigger you wish to create.
   *
   * @typedef {Object} AllowProcessHashOptions
   * @property {string} title - The title of the policy
   * @property {string} sha256 - The hash for which you wish to create a trigger
   * @property {string} [description] - The description of the policy
   * @property {boolean} [disable=false] - Whether or not to disable this policy upon creation
   ...
   */
  /**
   * Create a whitelist policy for processes with a given sha256 hash.
   *
   * @param {AllowProcessHashOptions} whitelistOptions - The options for the blacklist policy you wish to create
   * @return {Promise<ReaQtaResponse>}
   * @rqtResource Policy
  */
  allowProcessHash(whitelistOptions) {
    return this._signAndExecuteRequest(
      '/1/policy/allow/process-hash',
      {
        method: POST,
  ...
  /**
   * @typedef {Object} AllowFromAlertOptions
   * @property {string} alertId - The alert from which to make the whitelist policy
   * @property {string} title - The title of the policy
   * @property {string} description - The description of the policy
   */
  /**
   * Create a whitelist policy from a given alert
   *
   * @param {AllowFromAlertOptions} fromAlertOptions - The options for the blacklist policy you wish to create
   * @return {Promise<ReaQtaResponse>}
   * @rqtResource Policy
  */
  allowFromAlert(fromAlertOptions) {
    return this._signAndExecuteRequest(
      '/1/policy/allow/from-alert',
      {
        method: POST,
        body: fromAlertOptions
      }
    )
  }

  /**
   * Hunt events using the ReaQta Hunt Query Language
   * @param {String} huntQuery - A valid hunt query
   * @param {Object} params - Search criteria (consult external api documentation)
   * @return {Promise<ReaQtaResponse>}
   * @rqtResource Event
   */
  huntEvents(huntQuery, params = {}) {
    return this._signAndExecuteRequest(
      '/1/events/hunt',
      {
        method: POST,
        body: { query: huntQuery },
        params
      }
    )
  }
  ...
  /**
   * Get the details of a single alert.
   * @param {string} localId - The *local* id of the alert
   * @param {string} endpointId - The id of the endpoint on which the alert happened
   * @return {Promise<ReaQtaResponse>}
   * @rqtResource Alert
   */
  getAlertByLocalId({ localId, endpointId }) {
    return this._signAndExecuteRequest(
      `/1/alert/local/${localId}/endpoint/${endpointId}`
    )
  }

  /**
   * Search all events associated with a given alert.
   * Added in v3.8.3.
   * @param {Object} params - Search criteria (consult external api documentation)
   * @return {Promise<ReaQtaResponse>}
   * @rqtResource Alert
   */
  searchAlertEvents(alertId, params = {}) {
    return this._signAndExecuteRequest(
      `/1/alert/${alertId}/events`,
      { params }
    )
  }
   ...
   * @param {string} id - The id of the alert
   * @param {string} tag - The new tag for the alert
   * @return {Promise<ReaQtaResponse>}
   * @rqtResource Alert
   */
  addTagToAlert(id, tag) {
    return this._signAndExecuteRequest(
      `/1/alert/${id}/tags/${encodeURI(tag)}`,
      {
        method: POST
      }
    )
   ...
   * @param {string} id - The id of the alert
   * @param {string} tag - The tag to remove from the alert
   * @return {Promise<ReaQtaResponse>}
   * @rqtResource Alert
   */
  removeTagFromAlert(id, tag) {
    return this._signAndExecuteRequest(
      `/1/alert/${id}/tag/${encodeURI(tag)}`,
      {
        method: DELETE
      }
    )
   ...
   * @param {string} id - The id of the alert
   * @param {Object} params - notes string
   * @return {Promise<ReaQtaResponse>}
   * @rqtResource Alert
   */
  addNotesToAlert(id, notes) {
    return this._signAndExecuteRequest(
      `/1/alert/${id}/notes`,
      {
        method: PUT,
