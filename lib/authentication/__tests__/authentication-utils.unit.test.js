...
beforeAll(() => {
  ...
  jest.setSystemTime(new Date('2021-04-01'))
})
afterAll(() => {
  jest.useRealTimers()
})
...
test('shouldRefetchToken returns true if jwt is falsy', () => {
  const willRefetch = shouldRefetchToken(null, Date.now())
  expect(willRefetch).toBe(true)
})

test('shouldRefetchToken returns true if jwt expired 1ms ago', () => {
  const willRefetch = shouldRefetchToken('123', Date.now() - 1)
  expect(willRefetch).toBe(true)
})

test('shouldRefetchToken returns true if jwt expires within the next second', () => {
  const willRefetch = shouldRefetchToken('123', Date.now() + 999)
  expect(willRefetch).toBe(true)
})

test('shouldRefetchToken returns false if jwt is truthy and does not expire within next second', () => {
