 ...
 * @property {number} expiresAtMS - Time at which the current JWT expires, in milliseconds
 */

/**
 * Check if we have a non-expired JWT
 */
function shouldRefetchToken(jwt, expiresAtMS) {
  if (!jwt) {
    return true
  }
...
function refreshToken(client, tokenOptions, shouldRefetch = shouldRefetchToken, fetchToken = getJWT) {
  const { appId, appSecret, jwt, expiresAtMS } = tokenOptions

  if (!shouldRefetch(jwt, expiresAtMS)) {
