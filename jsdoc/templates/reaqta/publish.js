...
function buildResourcesNav(items, itemHeading, itemsSeen, linktoFn) {
    if (!items.length) {
        return ''
    }

    const itemsByResource = items.reduce((res, item) => {
        res[item.rqtResource] = res[item.rqtResource] || []
        res[item.rqtResource].push(item)
        return res
    }, {})

    let nav = '';

    const resources = Object.keys(itemsByResource)
    resources.sort()
    resources.forEach(resourceName => {
        let itemsNav = '';
        const itemsForResource = itemsByResource[resourceName]
        itemsForResource.forEach(item => {
            let displayName;
        ...
        if (itemsNav !== '') {
            nav += `<li><a class="rqt-resource" href="#">${resourceName}</a><ul class="dropdown">${itemsNav}</ul></li>`;
        }
    })

    nav = `<h3>${itemHeading}</h3><ul>${nav}</ul>`;
 ...
 * @param {array<object>} members.client
 * @param {array<object>} members.resources
 * @param {array<object>} members.misc
 * @param {array<object>} members.errors
...
function buildNav(members) {
    ...
    nav += buildMemberNav(members.client, 'Client', {}, linkto);
    nav += buildResourcesNav(members.resources, 'Api Resources', {}, linkto);
    nav += buildMemberNav(members.misc, 'Helpers', {}, linkto);
    nav += buildMemberNav(members.errors, 'Errors', {}, linkto);

    nav += buildMemberNav(members.modules, 'Modules', {}, linkto);
    nav += buildMemberNav(members.externals, 'Externals', seen, linktoExternal);
    nav += buildMemberNav(members.namespaces, 'Namespaces', seen, linkto);
    // NOTE: Remove "classes" from nav
    //
...
exports.publish = (taffyData, opts, tutorials) => {
    ...
    const taffyFind = (data, spec) => data(spec).get();
    members.client = [
        ...taffyFind(data, { name: 'ReaQtaClient' }),
        ...taffyFind(data, { name: 'ReaQtaResponse' }),        
    ]
    members.resources = taffyFind(data, { rqtResource: { isUndefined: false } })
    members.misc = taffyFind(data, { rqtHelper: true })
    members.errors = taffyFind(data, { rqtError: true })
